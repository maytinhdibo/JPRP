...
<body class="pkp_page_{$requestedPage|escape|default:"index"} pkp_op_{$requestedOp|escape|default:"index"}{if $showingLogo} has_site_logo{/if}">
	...
	{if $displayPageHeaderLogo && is_array($displayPageHeaderLogo)}
							<a href="{url router=$smarty.const.ROUTE_PAGE page="index"}" class="navbar-brand navbar-brand-logo">
	...
	{* User profile, login, etc, navigation menu*}
		<nav aria-label="{translate|escape key="common.navigation.user"}">
						{load_menu name="user" id="navigationUser" ulClass="nav nav-pills tab-list pull-right navbar-nav"}
					</nav>
						{* Search form *}
